    ...
    while True:
        ...
        for p in range(len(arguments[1:])):
            ...
            try:
                ...
                cpu_acc += cpu_usage(pid) / psutil.cpu_count()
                vms_acc += vms_usage(pid)
                rss_acc += memory_usage(pid)
                ...
                exit(0)
        cpu_matrix.append((cpu_acc, time.time()-start_time))
        mem_matrix.append((rss_acc, time.time() - start_time))
        ram_matrix.append((vms_acc, time.time() - start_time))
        time.sleep(0.1)